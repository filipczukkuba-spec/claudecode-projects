...
export default function RecipeInput({ items, setItems }: Props) {
  ...
  async function handleExtract() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || "Błąd podczas pobierania przepisu");
        return;
      }
    ...
    <div className="bg-white rounded-2xl shadow-sm p-5 mb-4">
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="bg-green-500 hover:bg-green-600 disabled:bg-gray-200 disabled:text-gray-400 text-white rounded-xl px-4 py-3 text-sm font-semibold transition-colors whitespace-nowrap"
        >
          {loading ? "..." : "Dodaj"}
        </button>
      </div>
      ...
      {added.length > 0 && (
        <p className="text-green-600 text-xs mt-2">
          Dodano {added.length} składnik{added.length === 1 ? "" : added.length < 5 ? "i" : "ów"}: {added.join(", ")}
        </p>
      )}
    </div>
  );

// APEX_SMC_EA_v2.mq4 — Full Confluence Smart Money Concepts EA
// All 8 confluences required — highest accuracy, fewer trades (~3-5/month)
//
   ...
   for (int i=3;i<50&&!sweepBull&&!sweepBear;i++) {
      for (int j=i+1;j<50;j++) {
         if (MathAbs(Low[i]-Low[j])<tol) {
            ...
            for (int k=1;k<i;k++) if(Low[k]<lvl-tol&&Close[1]>lvl){sweepBull=true;sweepLvl=lvl;if(ShowLabels)Label("Sw_B_"+TimeToStr(Time[i]),Time[i],Low[i]-20*_Point*10,"Sweep↑",clrAqua);break;}
            if(sweepBull)break;
         }
         if (MathAbs(High[i]-High[j])<tol) {
            double lvl=(High[i]+High[j])/2.0;
            for (int k=1;k<i;k++) if(High[k]>lvl+tol&&Close[1]<lvl){sweepBear=true;sweepLvl=lvl;if(ShowLabels)Label("Sw_S_"+TimeToStr(Time[i]),Time[i],High[i]+20*_Point*10,"Sweep↓",clrMagenta);break;}
            if(sweepBear)break;
         }
      }
   }
...
void FetchNews() {
   ...
   if(code!=200){Print("APEX v2 News: failed (code=",code,"). Add URL to allowed list.");return;}
   ...
   Print("APEX v2 News: ",newsCount," events loaded.");
}
...
void DrawBox(string name,datetime t1,double p1,datetime t2,double p2,color clr){
   ...
   ObjectSet(name,OBJPROP_COLOR,clr); ObjectSet(name,OBJPROP_BACK,true);
   ObjectSet(name,OBJPROP_FILL,true); ObjectSet(name,OBJPROP_WIDTH,1);
}
...
void DrawHLine(string name,double price,color clr,int style){
   ...
   ObjectSet(name,OBJPROP_COLOR,clr); ObjectSet(name,OBJPROP_STYLE,style); ObjectSet(name,OBJPROP_WIDTH,1);
}

void Label(string name,datetime t,double price,string txt,color clr){
   string n="APEX_"+name; if(ObjectFind(n)>=0) ObjectDelete(n);
   ObjectCreate(n,OBJ_TEXT,0,t,price);
   ObjectSetString(0,n,OBJPROP_TEXT,txt);
   ObjectSet(n,OBJPROP_COLOR,clr); ObjectSet(n,OBJPROP_FONTSIZE,8);
}
